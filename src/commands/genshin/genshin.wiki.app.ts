...
const language: QueryOptions = {
    queryLanguages: [Languages.ChineseSimplified],
    resultLanguage: Languages.ChineseSimplified,
};
...
class GenshinWiki extends AppCommand {
    ...
    help =
        '`.查询 类别（角色、天赋、命之座、材料、圣遗物、料理、怪物、武器） 名称`'; // 帮助文字
    ...
    func: AppFunc<BaseSession> = async (session) => {
        ...
        try {
            switch (session.args[0]) {
                case '角色':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('角色列表').addText(itemtext);
                    } else {
                        ...
                        card.addText(
                            ...
                            true,
                            'left',
                            { type: 'image', src: res.images.icon, size: 'lg' }
                        )
                            .addText(
                                `\`${res.element}\` \`${res.weapontype}\` \`${res.substat}\``
                            )
                ...
                case '天赋':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('天赋列表').addText(itemtext);
                    } else {
                ...
                case '命之座':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('命之座列表').addText(itemtext);
                    } else {
                        res = genshindb.constellations(
                            session.args[1],
                            language
                        );
                        card.addTitle(`命之座: ${res.name}`)
                            .addDivider()
                            .addText(
                                `**${res.c1.name}**\n${res.c1.effect}`,
                                true,
                ...
                case '材料':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('材料列表').addText(itemtext);
                    } else {
                        ...
                        var mattext: string = '',
                            mat: any;
                        for (mat of res.source) mattext += mat;
                        card.addText(`**获取方式：**${mattext}`).addText(
                            `[Fandom](${res.url.fandom})`
                        );
                    }
                    break;
                case '圣遗物':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('圣遗物列表').addText(itemtext);
                    } else {
                ...
                case '料理':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('料理列表').addText(itemtext);
                    } else {
                ...
                case '怪物':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('怪物列表').addText(itemtext);
                    } else {
                ...
                case '武器':
                    if (!session.args[1]) {
                        ...
                        var item: any,
                            itemtext: string = '';
                        for (item of res) itemtext += `${item}\n`;
                        card.addTitle('武器列表').addText(itemtext);
                    } else {
                        ...
                        var effect = res.effect.replace(
                            '{0}',
                            res.r1 + '~' + res.r5
                        );
                        card.addText(`**${res.name}**\n${rare}`)
                            //,true,'left',{type: "image", src: res.images.image ,size: "lg"})
                            .addText(
                                `\`${res.weapontype}\` \`${res.substat} : ${res.subvalue}\``
                            )
                            .addText(effect)
                            .addText(`> ${res.description}`)
                            .addText(`[Fandom](${res.url.fandom})`);
            ...
            console.log(error)
            return session.sendCard(
                new Card().addTitle(`无条目「${session.args[1]}」`)
            );
        }
        if (res == undefined)
            return session.sendCard(
