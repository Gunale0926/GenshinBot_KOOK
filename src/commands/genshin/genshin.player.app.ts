...
class GenshinPlayer extends AppCommand {
    ...
    func: AppFunc<BaseSession> = async (session) => {
        ...
        genshin.getUserInfo(uid).then(function (res: any) {
            ...
            for (avatar of res.avatars) {
                card.addText(
                    ...
                    true,
                    'left',
                    { type: 'image', src: avatar.image, size: 'sm' }
                );
            }
